...
st.title("Tool for the assessment of overheating in Italian school buildings")
st.write("This tool allows to analyse the overheating potential of the entire school building, on the basis of the location and of the geometrical, envelope and ventilation characteristics.")

st.subheader("School building data", divider = True)
climatic_zone = st.radio("Select a climatic zone", ["A", "B", "C", "D", "E", "F"], index = None, horizontal = True)
numeroAULE = st.number_input("Insert the total number of classrooms in the school:", min_value = 0)
number_of_types = st.slider("Select the number of classroom groups, clustered for floor, position and orientation", 0, 10)
...
st.write("Solution for school in climatic zone {}, with WFR {}%, SHGC {} and {}".format(climatic_zone, WFR, valore, retrofitCaso))
st.write("For each group, select the number of classrooms, the postion and the orientation.")
...
heat_stress = pd.DataFrame(columns = ["total", "school non heating"], index = range(number_of_types))
heat_stress_tot = pd.DataFrame(columns = ["total", "school non heating"], index = range(number_of_types))

for gruppo in range(number_of_types):
    st.subheader("\nClassroom group {}".format(gruppo+1), divider = True)
    if gruppo == number_of_types-1:
        numero = numeroAULE
        st.write("Number of classrooms that belong to the group {}: {}".format(gruppo+1, numero))
    else:
        numero = st.slider("Select the number of classrooms that belong to the group {}".format(gruppo+1), 0, numeroAULE)
    
    piano = st.radio("Select the floor of classroom group {}".format(gruppo+1), ["Ground", "Middle", "Top"], index = None, horizontal = True)
    pos = st.radio("Select the position of classroom group {}".format(gruppo+1), ["Central", "Angular"], index = None, horizontal = True)
    card = st.radio("Select the orientation of classroom group {}".format(gruppo+1), ["South", "West", "North", "East"], index = None, horizontal = True)
    
    button1 = st.toggle("Confirm", key = "Carica{}".format(gruppo))
    if not button1:
        st.stop()
        
    numeroAULE = numeroAULE - numero
    numero_df.loc[gruppo, "nome"] = "Classrooms at {} floor, {}, {} exposed".format(piano, pos, card) 
...
# %% Read files
st.subheader("Loading...", divider = True)
st.write("Estimated time: {} min {} sec".format(math.floor(number_of_types*20/60), round(number_of_types*20%60)))
...
for gruppo in range(number_of_types):
    ...
    st.write("Loading - Group {}...".format(gruppo+1))
...
#%% Comfort bands
st.write("Share of hours with T>Tmax in the different periods of the year")
comfort_bands.columns = ["Start of year - \nend of \nheating season", "End of \nheating season - \nend of school", "End - start \nof school", "Start of school - \nstart of \nheating season", "Start of \nheating season - \nend of year"]
...
ax.text(0.5, -1.05, "N° of hours with T>Tmax / N° of hours tot", ha = "center")
...
fig.set_figheight(6)
fig.set_figwidth(9)
ax.text(0.5, -1.05, "N° of hours with T>Tmax+2°C / N° of hours tot", ha = "center")
...
st.write("The school presents {} / {} classrooms at overheating risk according to TM52".format(round(somma), (numero_df["numero"]).sum()))
...
#%% Heat stress
st.write("Total heat stress days and heat stress days for the non heating school period.")
...
rischio["comfort bands"] = rischio["comfort bands"].mask((comfort_bands['End of \nheating season - \nend of school'] > 5) | (comfort_bands['Start of school - \nstart of \nheating season'] > 5), 1)
rischio["dist"] = rischio["dist"].mask((ratio['school non heating'] > 0.05), 1)
rischio["tm52"] = rischio["tm52"].mask((tm52['TOT'] == 100), 1)
rischio["heat stress"] = rischio["heat stress"].mask((ratio2['school non heating'] > 0.1), 1)
...
rischio["rate"] = rischio["rate"].mask(rischio["finale"] == 1, "high")
rischio["rate"] = rischio["rate"].mask(rischio["finale"] == 0.5, "middle")
rischio["rate"] = rischio["rate"].mask(rischio["finale"] == 0, "low")

for gruppo in range(number_of_types):
    if rischio["rate"][gruppo] == "high":
        st.write("Group {}: {} {} --> :red[{}] overheating risk in the intermediate seasons".format(gruppo+1, numero_df["numero"][gruppo], numero_df["nome"][gruppo], rischio["rate"][gruppo]))
    elif rischio["rate"][gruppo] == "middle":
        st.write("Group {}: {} {} --> :yellow[{}] overheating risk in the intermediate seasons".format(gruppo+1, numero_df["numero"][gruppo], numero_df["nome"][gruppo], rischio["rate"][gruppo]))      
    elif rischio["rate"][gruppo] == "low":
        st.write("Group {}: {} {} --> :green[{}] overheating risk in the intermediate seasons".format(gruppo+1, numero_df["numero"][gruppo], numero_df["nome"][gruppo], rischio["rate"][gruppo]))
...
rischio["comfort bands"] = rischio["comfort bands"].mask((comfort_bands["End - start \nof school"] > 5), 1)
rischio["dist"] = rischio["dist"].mask((ratio['non heating'] > 0.05), 1)
rischio["tm52"] = rischio["tm52"].mask((tm52['TOT'] == 100), 1)
rischio["heat stress"] = rischio["heat stress"].mask((ratio2['total'] > 0.1), 1)
...
for gruppo in range(number_of_types):
    if rischio["rate"][gruppo] == "high":
        st.write("Group {}: {} {} --> :red[{}] overheating risk in summer".format(gruppo+1, numero_df["numero"][gruppo], numero_df["nome"][gruppo], rischio["rate"][gruppo]))
    elif rischio["rate"][gruppo] == "middle":
        st.write("Group {}: {} {} --> :yellow[{}] overheating risk in summer".format(gruppo+1, numero_df["numero"][gruppo], numero_df["nome"][gruppo], rischio["rate"][gruppo]))      
    elif rischio["rate"][gruppo] == "low":
        st.write("Group {}: {} {} --> :green[{}] overheating risk in summer".format(gruppo+1, numero_df["numero"][gruppo], numero_df["nome"][gruppo], rischio["rate"][gruppo]))
